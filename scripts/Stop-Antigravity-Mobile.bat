...
REM Stop HTTP server on port 3333
echo Checking HTTP server (port 3333)...
set "FOUND_HTTP=0"
for /f "tokens=5" %%a in ('netstat -aon ^| findstr :3333 ^| findstr LISTENING 2^>nul') do (
...
if "%FOUND_HTTP%"=="0" (
    echo No HTTP server found running on port 3333.
)
